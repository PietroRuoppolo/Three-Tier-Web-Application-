...
resource "aws_security_group" "web_sg_east" {
  ...
  # HTTP/HTTPS da Internet (ok per frontend pubblico)
  ingress {
    from_port   = 80
    to_port     = 80
    protocol    = "tcp"
...
resource "aws_instance" "bastion_east" {
  ...
  key_name                    = aws_key_pair.deployer_east.key_name
  subnet_id                   = aws_subnet.prod["pub-sub-1a"].id # subnet pubblica
  vpc_security_group_ids      = [aws_security_group.bastion_east.id]
  associate_public_ip_address = true
  tags                        = { Name = "bastion-east" }
}

# Bastion EC2 (west / DR)
resource "aws_instance" "bastion_west" {
  provider                    = aws.secondary
  ami                         = data.aws_ami.ubuntu_west.id
  instance_type               = "t3.micro"
  key_name                    = aws_key_pair.deployer_west.key_name
  subnet_id                   = aws_subnet.dr["pub-sub-1a"].id # subnet pubblica
  vpc_security_group_ids      = [aws_security_group.bastion_west.id]
  associate_public_ip_address = true
  tags                        = { Name = "bastion-west" }
}


# EAST: consenti SSH solo dal SG del bastion EAST verso il SG web EAST
resource "aws_security_group_rule" "ssh_from_bastion_to_web_east" {
  provider                 = aws.primary
  type                     = "ingress"
  security_group_id        = aws_security_group.web_sg_east.id
  source_security_group_id = aws_security_group.bastion_east.id
  from_port                = 22
  to_port                  = 22
  protocol                 = "tcp"
}

# WEST/DR: consenti SSH solo dal SG del bastion WEST verso il SG web WEST
resource "aws_security_group_rule" "ssh_from_bastion_to_web_west" {
  provider                 = aws.secondary
  type                     = "ingress"
  security_group_id        = aws_security_group.web_sg_west.id
  source_security_group_id = aws_security_group.bastion_west.id
  from_port                = 22
  to_port                  = 22
  protocol                 = "tcp"
}
